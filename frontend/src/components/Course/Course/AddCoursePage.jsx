...
import { notify } from "../../../utils/shared/notifier";
import { useAddCourseForm } from "../../../features/course/hooks/useAddCourseForm";
import { useCategoryDialog } from "../../../features/course/hooks/useCategoryDialog";
import AddCourseForm from "./partials/AddCourseForm";
import CategoryDialog from "./partials/CategoryDialog";
import ThumbnailUploader from "./partials/ThumbnailUploader";
...
const AddCourse = () => {
  ...
  const {
    ...
    handleCategoryInput: handleInputCategory,
    handleCategorySubmit: handleCategory,
  } = useCategoryDialog({
  ...
  const {
    ...
    onAddCategoryRequested: handleClickOpen,
    onValidSubmit: async (payload) => {
      try {
        dispatch(createCoursesRequest(payload));
      } catch (error) {
  ...
  useEffect(() => {
    if (addCategorySuccessState) {
      handleClose();
      notify({ icon: "success", title: "Category added successfully" });
  ...
  useEffect(() => {
    if (addCategoryFailureState) {
      ...
      // // Clear the timeout if the component unmounts
      // return () => clearTimeout(timer);
      notify({ icon: "warning", title: "Category already exists" });
  ...
  useEffect(() => {
    if (InternalError) {
      ...
      // // Clear the timeout if the component unmounts
      // return () => clearTimeout(timer);
      notify({ icon: "error", title: "Internal Server error occured" });
    }
  }, [InternalError]);
  ...
  useEffect(() => {
    if (isExist) {
      notify({ icon: "warning", title: "Course already exists" });
 ...
 /> */}
      ...
      <CategoryDialog
        ...
        onSubmit={handleCategory}
        category={category}
        onCategoryChange={handleInputCategory}
