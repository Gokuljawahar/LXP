...
import {
  FETCH_QUIZ_REQUEST_USERS,
  fetchquizpassedusersSuccess,
  fetchquizpassedusersFailure,
} from "../../../actions/Admin/QuizPassedUserAction";

const ApiQuizPassedUsers =
  ({ dispatch }) =>
  (next) =>
  async (action) => {
    next(action);
    ...
    if (action.type === FETCH_QUIZ_REQUEST_USERS) {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5199/api/QuizReport/QuizReport/passedlearnersReport/${quizid.quizId}!`
        );
